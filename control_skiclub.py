...
if __name__ == "__main__":
    ...
    df_hill = read_data('skihill_digitize.shp').to_crs('ESRI:102001')
    #print(df_underlying)
    ...
    df_sites = df_sites[df_sites['Name'] == "SkiClub"]
...
##    if len(df_clip) == 0:
    ...
    for n,polygon in zip(df_explode['Name'],list(df_explode['geometry'])):
        ...
        if n != 'None': 
            ...
            points1 = gpd.overlay(points, df_affected, how='difference')
            points2 = gpd.overlay(points1, df_hill, how='difference')
            points = points2.clip(polygon)
            
            polygon0 = polygon
            polygon1 = polygon0.difference(df_affected['geometry'].unary_union)
            polygon2 = gpd.GeoSeries(polygon1,crs='ESRI:102001').difference(df_hill['geometry'])
            #polygon2.to_file(driver = 'ESRI Shapefile', filename= "poly_skihill.shp")
...
##                    points = points.clip(gpd.GeoDataFrame(geometry=df_clip_proj,crs='ESRI:102001'))
    ...
    a.to_csv('dryden_500_control_5m_skiClub_nohill_seed42_correct.csv', index=False)
